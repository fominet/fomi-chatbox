...
setup:
	docker compose -f docker-compose.production.yaml run --rm -v $(CURDIR)/patch_migration.rb:/tmp/patch_migration.rb rails sh -c "ruby /tmp/patch_migration.rb && bundle exec rails db:create db:migrate"
